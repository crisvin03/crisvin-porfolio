...
const cvData = {
  ...
  projects: [
    ...
    { name: "BuMo", link: "https://github.com/crisvin03/bumo", summary: "Mobile-first experiment / utilities bundle. A Ride Sharing App.", tech: ["React Native", "Expo"] },
  ],
...
function buildCVHtml(d) {
  ...
  <head>
    ...
    <style>
      ...
      *{ box-sizing:border-box; }
      ...
      /* Sidebar */
      .avatar{ width:76px; height:76px; border-radius:12px; border:1px solid var(--line); background:#0b1220; display:grid; place-items:center; color:#74809a; margin:0 auto 8px; font-size:11px; }
      .section-title{ margin:10px 0 6px; font-weight:800; font-size:11px; letter-spacing:.12em; text-transform:uppercase; color:#c7d2fe; }
      .section-title.profile-title{ text-align:center; }
  ...
  <body>
    <div class="sheet">
      ...
      <div class="grid">
        <aside class="panel">
          <div class="section-title profile-title">Profile</div>
          ${d.avatarUrl
...
/* ---------------- Download handler ---------------- */
// PDF file path - update this to match your PDF filename in the public folder
const CV_PDF_PATH = '/CV_Crisvin_Habitsuela.pdf'; // Change this to your PDF filename

function handleDownloadCV() {
  // Create a temporary anchor element to download the PDF
  const link = document.createElement('a');
  link.href = CV_PDF_PATH;
  link.download = CV_PDF_PATH.split('/').pop(); // Get filename from path
...
export default function Contact() {
  ...
  // Auto-hide success message when user returns to tab
  useEffect(() => {
    if (submitStatus === 'success') {
      ...
      const handleVisibilityChange = () => {
        // When user comes back to the tab, wait 3 seconds then hide message
        if (!document.hidden) {
          hideMessage();
        } else {
          // User switched away, clear any existing timer
          if (timer) {
            clearTimeout(timer);
            timer = null;
          }
      ...
      if (!document.hidden) {
        // User is already on the tab, hide after 3 seconds
        hideMessage();
      }
      
      // Listen for when user comes back or switches away
  ...
  const handleSubmit = (e) => {
    ...
    // Construct Gmail compose URL with pre-filled information in professional email format
    const subject = encodeURIComponent(`Inquiry from ${formData.name} - Portfolio Contact Form`);
    const body = encodeURIComponent(
      `Dear Crisvin,\n\n` +
      `I hope this message finds you well. I am reaching out through your portfolio contact form.\n\n` +
      `${formData.message}\n\n` +
      `---\n` +
      `Contact Information:\n` +
      `Name: ${formData.name}\n` +
    ...
    const gmailComposeUrl = `https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(cvData.email)}&su=${subject}&body=${body}`;
  ...
  return (
    <section id="contact" className="scroll-mt-24 py-16 sm:py-20 w-full overflow-x-hidden">
            ...
            <form onSubmit={handleSubmit} className="grid gap-4">
              ...
              {submitStatus === 'success' && (
                ...
                >
                  ...
                  <span className="text-sm">Your email has been sent successfully!</span>
                </motion.div>
              )}
